...
async function handler(req: any, res: any) {
  ...
  if (!url || typeof url !== "string" || !/^https?:\/\/.+/i.test(url)) {
    return res.status(400).json(apiError(ErrorCode.INVALID_INPUT, "A valid image URL is required.", undefined, version));
  }
  ...
  try {
    ...
    if (!headRes.ok) {
      return res.status(400).json(apiError(ErrorCode.INVALID_INPUT, `Failed to fetch image: HTTP ${headRes.status}`, undefined, version));
    }

    const contentType = headRes.headers.get("content-type");
    if (!contentType || !ALLOWED_MIME_TYPES.some(type => contentType.toLowerCase().includes(type))) {
      return res.status(400).json(apiError(ErrorCode.INVALID_INPUT, `Unsupported content type: ${contentType}. Must be JPEG, PNG, WebP, or GIF.`, undefined, version));
    }

    const contentLength = headRes.headers.get("content-length");
    if (contentLength && parseInt(contentLength, 10) > MAX_IMAGE_BYTES) {
      return res.status(400).json(apiError(ErrorCode.INVALID_INPUT, `Image is too large. Max size is 5MB.`, undefined, version));
    }

    return res.status(200).json(withVersion({ valid: true, contentType, contentLength }, version));
  } catch (error: any) {
    if (req.logger?.error) {
    ...
    return res.status(500).json(apiError(ErrorCode.TEMPORARY_FAILURE, "Failed to reach the remote image URL.", undefined, version));
  }
}
