...
export const CURRENT_API_VERSION = "2025-01-01" as const;
...
export const WEBHOOK_SCHEMA_VERSION = "2025-01-01" as const;
...
/**
 * Mixin added to every successful API response envelope.
 * Error responses produced by `apiError()` also carry this field so clients
 * can always read the version regardless of HTTP status code.
 */
...
export const ACCEPT_VERSION_HEADER = "accept-version" as const;
...
export function resolveApiVersion(
  ...
  const normalised = requested.trim() as ApiVersion;
  if ((SUPPORTED_API_VERSIONS as readonly string[]).includes(normalised)) {
    return normalised;
  }
 ...
 */
export const UNSUPPORTED_VERSION_CODE = "UNSUPPORTED_VERSION" as const;
export type UnsupportedVersionCode = typeof UNSUPPORTED_VERSION_CODE;
